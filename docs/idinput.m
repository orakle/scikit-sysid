...
elseif strcmpi(type,'prbs')
   ...
   if (clockP*P1~=P)
      if M>1
         ctrlMsgUtils.warning('Ident:dataprocess:idinput6',clockP*P1,P1*M*clockP)
         P = P1*clockP;
      else
         n = min(n+1,18);
         P1 = 2^n -1;
         ctrlMsgUtils.warning('Ident:dataprocess:idinput7',P,clockP*P1)
      end
   end
   ...
   if n<3 || n>18,
      ctrlMsgUtils.error('Ident:dataprocess:idinput8')
   end
   fi = -ones(n,1);
   if n==3
      ind = [1,3];
